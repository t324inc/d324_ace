...
namespace Drupal\d324_ace\Plugin\Field\FieldWidget;
...
use Drupal\d324_ace\Event\AcePropertiesFormEvent;

/**
 * D324 Ace field widget.
 *
 * @FieldWidget(
 *   id = "d324_ace_widget",
 *   label = @Translation("D324 Ace (With layout builder)"),
 ...
 * )
 */
class AceWidget extends InlineEntityFormComplex {
  ...
  /**
   * Constructs an AceWidget object.
   *
  ...
  public function formElement(FieldItemListInterface $items, $delta, array $element, array &$form, FormStateInterface $form_state) {
    ...
    $element['entities']['#theme'] = 'd324_ace_widget';
    $entities = $form_state->get([
      'inline_entity_form',
      $this->getIefId(),
      'entities',
    ]);
    $element['entities']['#widget'] = 'd324_ace_widget';
    ...
    foreach ($bundles as $bundle) {
      ...
      if (method_exists($entity, 'getParagraphType')) {
        if ($icon = $entity->getParagraphType()->getIconFile()) {
          $path = $icon->url();
          $element['entities']['#attached']['drupalSettings']['aceIcons']['icon_' . $bundle] = $path;
        }
      }
    }
    ...
    if (isset($element['actions'])) {
      ...
      $element['actions']['#attributes']['class'][] = 'ace-field-actions';
      ...
      $element['actions']['region'] = [
        '#type' => 'hidden',
        '#attributes' => [
          'class' => ['ace-new-item-region', 'hidden'],
        ],
      ];
      $element['actions']['delta'] = [
        '#type' => 'hidden',
        '#attributes' => [
          'class' => ['ace-new-item-delta', 'hidden'],
        ],
      ];
    }

    if ($this->getSetting('full_screen_editing')) {
      $element['entities']['#attached']['library'][] = 'd324_ace/full_screen_editing';
    }
    ...
    foreach ($entities as $delta => $value) {
      ...
      if (!empty($form_item['form'])) {
        if ($value['form'] == 'remove') {
          try {
            $form_item['form']['#attributes']['class'][] = 'ace-remove-form';
            ...
            if ($bundle_label) {
              ...
              $form_item['form']['message']['#attributes']['class'] = ['ace-remove-message'];
            }
          }
          catch (\Exception $e) {
      ...
      if (!empty($form_item['form']['inline_entity_form'])) {
        ...
        $form_item['form']['#process'][] = 'd324_ace_alter_form';
        $form_item['form']['#theme'] = 'd324_ace_ief';
        ...
        $form_item['form']['inline_entity_form']['#ace_form'] = TRUE;
      }
    ...
    if (isset($element['form']['inline_entity_form'])) {
      ...
      $element['form']['#theme'] = 'd324_ace_ief';
      $element['form']['#process'][] = 'd324_ace_alter_form';
      $element['form']['#attributes']['class'][] = 'ace-add-entity-form';
      $element['form']['inline_entity_form']['#ace_form'] = TRUE;
  ...
  public static function alterEntityFormActions($element) {
    ...
    $element['actions']['#attributes']['class'][] = 'ace-form-actions';
   ...
   *   see https://www.drupal.org/project/d324_ace/issues/3038533
   */
    ...
    $element['layout_properties']['layout'] = [
      ...
      '#attributes' => [
        'class' => [
          'ace-layout-select',
          'ace-layout',
          'ace-layout-' . (!empty($default_properties['layout']) ? $default_properties['layout'] : ''),
        ],
      ],
    ];
    $element['layout_properties']['region'] = [
      ...
      '#attributes' => [
        'class' => [
          'ace-region-select',
          'ace-region',
          'ace-region-' . (!empty($default_properties['region']) ? $default_properties['region'] : ''),
        ],
      ],
    ];
  ...
  public static function processLayoutOptions($element) {
    foreach (Element::children($element) as $radio_item) {
      $element[$radio_item]['#theme_wrappers'][] = 'd324_ace_radio';
    }
    return $element;
  }
  ...
  public function layoutOptionsForm($element, $form_state) {
    ...
    $element['layout_selection']['layout'] = [
      ...
      '#attributes' => [
        'class' => ['ace-layout', 'ace-layout-section-select'],
      ],
      '#required' => TRUE,
    ...
    $event = new AcePropertiesFormEvent($element, $defaults);
    $this->eventDispatcher->dispatch(AcePropertiesFormEvent::EVENT_NAME, $event);
