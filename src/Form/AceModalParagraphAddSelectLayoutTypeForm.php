 ...
 */
class AceModalParagraphAddSelectLayoutTypeForm extends FormBase {

  /**
   * {@inheritdoc}
   */
  public function getFormId() {
    return 'ace_modal_add_select_layout_type_form';
  }
  ...
  public function buildForm(array $form, FormStateInterface $form_state) {
    ...
    $default_icon = drupal_get_path('module', 'd324_ace') . '/img/d324-ace-puzzle.svg';

    foreach ($bundles as $key => $bundle) {
      if(!in_array($bundle['name'], $settings['layout_bundles'])) {
        ...
        $form['description'][$bundle_name] = [
          '#type' => 'image_button',
          '#prefix' => '<div class="d324-ace-frontend-add-type">',
          '#suffix' => '<span>' . $label . '</span></div>',
          '#src' => $icon_url,
          '#value' => $label,
          '#ajax' => [
            'url' => Url::fromRoute(isset($routeParams['paragraph'])? 'd324_ace.modal.add_form': 'd324_ace.modal.add_form_first', $routeParams),
            'wrapper' => 'd324-ace-frontend-modal-form',
          ],
        ];
      }
    }
