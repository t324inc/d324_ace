 ...
 *
 * @ParagraphsBehavior(
 *   id = "ace_paragraphs_styles",
 *   label = @Translation("Ace Paragraph Styles"),
 *   description = @Translation("Allows the selection of a pre-defined visual style for a whole paragraph."),
 *   weight = 0
 * )
 */
class AceParagraphsStylesBehavior extends ParagraphsBehaviorBase implements ContainerFactoryPluginInterface {
   ...
   *
   * @var \Drupal\d324_ace_paragraph_behaviors\StyleDiscovery
   */
   ...
   * @param \Drupal\d324_ace_paragraph_behaviors\StyleDiscoveryInterface $yaml
   *   The yaml style discovery.
  ...
  public function buildBehaviorForm(ParagraphInterface $paragraph, array &$form, FormStateInterface $form_state) {
    $form['#attached']['library'][] = 'd324_ace_paragraph_behaviors/plugin_admin';

    // Create a unique id for the wrapper.
    $wrapper_id = Html::getUniqueId('style-wrapper');
    $form['#type'] = 'details';
    $form['#title'] = $this->t('Preset Paragraph Styles');
    $form['#open'] = FALSE;
    $form['#weight'] = 0;
    $form['style_wrapper'] = [
      '#type' => 'container',
  ...
  public function submitBehaviorForm(ParagraphInterface $paragraph, array &$form, FormStateInterface $form_state) {
    ...
    if(!empty($form_state->getValue('style_wrapper'))) {
  ...
  public function view(array &$build, Paragraph $paragraph, EntityViewDisplayInterface $display, $view_mode) {
    ...
    foreach ($this->configuration['groups'] as $key => $value) {
      ...
      if ($style) {
        $build['#attributes']['class'][] = 'ace-paragraphs-behavior-' . $this->getPluginId() . '--' . $style['name'];
  ...
  public function preprocess(&$variables) {
    parent::preprocess($variables); // TODO: Change the autogenerated stub
  }
  ...
  public static function ajaxStyleSelect(array $form, FormStateInterface $form_state) {
    ...
    $style_discovery = \Drupal::getContainer()->get('d324_ace_paragraph_behaviors.style_discovery');
  ...
  public function getStyles(ParagraphInterface $paragraph) {
    ...
    if ($styles_config = $paragraph->getBehaviorSetting('ace_paragraphs_styles', 'styles')) {
